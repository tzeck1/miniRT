...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/08/02 16:26:21 by rsiebert          #+#    #+#             */
/*   Updated: 2022/08/02 16:26:22 by rsiebert         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	cy_counter(char *rt_file_path)
{
	...
	while (line != NULL)
	{
		...
		while (line[j] == ' ')
			j++;
		if (ft_strncmp(&line[j], CYLINDER_ID, ft_strlen(CYLINDER_ID)) == 0)
			count++;
		free(line);
		line = get_next_line(fd);
	}
...
{
	cy_node->id = CYLINDER;
	cy_node->center = vec_from_str(data[1], data[2], data[3]);
	cy_node->dir = vec_from_str(data[4], data[5], data[6]);
